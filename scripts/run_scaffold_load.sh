#!/bin/bash
# In-container wrapper for the scaffold seed loader. Activates the
# editable-installed venv and hands off to the `scripts.run_scaffold_load`
# module entry point.
#
...
# `scripts.run_scaffold_load` (and `scripts/__init__.py`) resolve.
set -e
cd /usr/app
VENV="${VIRTUAL_ENV:-/usr/app/.venv}"
export VIRTUAL_ENV="$VENV"
export PATH="${VENV}/bin:${PATH}"
exec python -m scripts.run_scaffold_load
