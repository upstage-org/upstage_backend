...
sudo cp -r ../pyproject.toml /app_code_dev
sudo cp -r ../uv.lock /app_code_dev
sudo cp -r ../src /app_code_dev
sudo cp -r ../alembic /app_code_dev
sudo cp -r ../scripts /app_code_dev
sudo cp -r ../dashboard/demo /app_code_dev
sudo cp -r ../migration_scripts /app_code_dev

currwd=`pwd`
cd /app_code_dev
sudo uv sync --no-dev 
sudo chown -R 1000:1000 /app_code_dev
cd $currwd
